...
class WorkerMixin:
    ...
    def _append_table_row(self, r: ResultRow) -> None:
        ...
        set_col(0, r.source)
        set_col(1, r.genre or "—")
        set_col(2, r.variant)
        set_col(3, size_mb, True)
        set_col(4, fmt_val(r.lsd_db, "{:.3f}"), True)
        set_col(5, fmt_val(r.snr_db, "{:.3f}"), True)
        set_col(6, fmt_val(r.spec_conv, "{:.3f}"), True)
        set_col(7, fmt_val(r.rmse, "{:.5f}"), True)
        set_col(8, fmt_val(r.si_sdr_db, "{:.3f}"), True)
        set_col(9, fmt_val(r.spec_centroid_diff_hz, "{:.3f}"), True)
        set_col(10, fmt_val(r.spec_cosine, "{:.4f}"), True)
        set_col(11, fmt_val(r.stoi, "{:.3f}"), True)
        set_col(12, fmt_val(r.pesq, "{:.2f}"), True)
        set_col(13, fmt_val(r.mos, "{:.2f}"), True)
        set_col(14, fmt_val(r.score, "{:.4f}"), True)
        set_col(15, fmt_val(r.time_sec, "{:.3f}"), True)
        self.table.setItem(row, 16, QTableWidgetItem(r.path))
